...
const currentUserName = ref('') // Veritabanından gelen isim
...
onMounted(() => {
  ...
  onAuthStateChanged(auth, async (user) => {
    if (user) {
      ...
      // İsmi veritabanından (users koleksiyonundan) çek
      try {
        ...
        if (docSnap.exists()) {
          ...
          // Veritabanında yoksa Auth ismini kullan
          currentUserName.value = user.displayName?.split(' ')[0] || 'Misafir'
        }
      } catch (e) {
        currentUserName.value = user.displayName?.split(' ')[0] || 'Misafir'
      }
...
const handleLogout = async () => {
  const auth = getAuth()
  await signOut(auth)
  window.location.href = '/' // Sayfayı tamamen yenileyerek anasayfaya git
}

// MENÜ DATALARI (Senin orijinal verilerin)
const categories = [
...
const newYearSubItems = ['Öne Çıkanlar', 'Makyaj', 'Parfüm', 'Cilt Bakım', 'Saç Bakım', 'Özel Setler', 'Kadın Hediye Önerileri', 'Erkek Hediye Önerileri', "%50'ye varan", 'Sephora Collection']
...
const sephoraMenuData = {
  col1: {
    topLinks: [{ text: 'Gift Finder', isBold: true }, { text: 'Tüm Ürünler', isBold: true }, { text: 'Özel Setler', isBold: true, isPink: true }, { text: 'Sephora Collection', isBold: true }, { text: 'En Yeniler', isBold: true }, { text: 'Çok Satanlar', isBold: true }, { text: 'Sosyal Medya Favorileri', isBold: true }],
    group: { title: 'Makyaj', items: ['Ten', 'Göz', 'Dudak', 'Kaş', 'Makyaj Setleri'] }
  },
  col2: [{ title: 'Fırça ve Aksesuarlar', items: ['Tüm Ürünler', 'Makyaj Aksesuarları', 'Cilt Aksesuarı', 'Vücut ve Banyo Aksesuarları', 'Saç Bakım Aksesuarları'] }, { title: 'Cilt Bakım', items: ['Serum', 'Günlük Nemlendirici', 'Gece Nemlendiricisi', 'Göz Kremi', 'Dudak Bakım', 'Maske', 'Cilt & Makyaj Temizleyiciler', 'Cilt Bakım Setleri'] }],
  col3: [{ title: 'Vücut ve Banyo', items: ['Vücut Bakım', 'Duş ve Banyo Bakım', 'Vücut ve Banyo Setleri'] }, { title: 'Saç Bakım', items: ['Şampuan', 'Maske', 'Saç Bakım', 'Saç Bakım Setleri'] }]
}
      ...
      <header class="main-header">
        ...
        <div class="user-actions">
               ...
               <div class="dropdown-inner">
                ...
                <div class="dropdown-content">
                  ...
                  <div class="col-right">
                    ...
                    <p class="description">Mağazada sunduğumuz hizmetleri keşfet!</p>
          ...
          <div class="action-wrapper" :class="{ 'menu-open': isUserMenuOpen }" @mouseenter="isUserMenuOpen = true" @mouseleave="isUserMenuOpen = false">
            ...
            <NuxtLink :to="currentUser ? '/favorites' : '/login'" class="action-item">
              <img src="~/assets/images/user-icon.png" alt="User" class="icon-img" />
              <span class="action-text">
                {{ currentUser ? 'Hesabım' : 'Giriş yap / Üye ol' }}
              </span>
            </NuxtLink>
            ...
            <div v-if="isUserMenuOpen" class="mega-dropdown user-dropdown-pos">
              <div class="dropdown-inner">
                ...
                <div class="user-dropdown-content">
                  ...
                  <div v-if="!currentUser" class="user-col-welcome">
                    <h3>Merhaba!</h3>
                  ...
                  <div v-else class="user-col-welcome logged-in-col">
                    <h3>Merhaba {{ currentUserName }} !</h3>
                    <p class="points-text">
                      Hesabında <strong>0 puan</strong> var. Bir sonraki %10 indirim kuponuna 15000 puan kaldı!
                    </p>
                    <a href="#" class="card-link">Sephora Kart ayrıcalıklarını keşfet</a>
                  ...
                  <div class="user-col-list border-left">
                    <ul class="icon-list">
                      <li><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5"><path d="M20 21v-2a4 4 0 0 0-4-4H8a4 4 0 0 0-4 4v2"/><circle cx="12" cy="7" r="4"/></svg><NuxtLink to="/favorites">Hesabım</NuxtLink></li>
...
/* GİRİŞ YAPMIŞ KULLANICI STİLİ */
.logged-in-col h3 { margin-bottom: 10px; }
.points-text { font-size: 13px; line-height: 1.5; color: #000; margin-bottom: 20px; }
.card-link { font-size: 13px; font-weight: 700; text-decoration: underline; color: #000; display: block; margin-bottom: 25px; }
.logout-wrapper { margin-top: 10px; }
.btn-logout { background: none; border: none; color: #D3004C; font-weight: 700; font-size: 14px; cursor: pointer; padding: 0; }
...
.nav-item > a { text-decoration: none; color: #000; font-size: 14px; font-weight: 500; white-space: nowrap; letter-spacing: 0.2px; padding-bottom: 15px; border-bottom: 3px solid transparent; }
...
.nav-item > a:not(.pink-text):hover, .nav-item > a:not(.pink-text).active-link { color: #000 !important; border-bottom: 3px solid #000 !important; }
...
.vertical-menu a, .clean-list a { border-bottom: none !important; padding-bottom: 0 !important; text-decoration: none; color: #000; font-size: 14px; font-weight: 700; }
.vertical-menu a:hover, .clean-list a:hover { color: #000 !important; text-decoration: underline !important; }
